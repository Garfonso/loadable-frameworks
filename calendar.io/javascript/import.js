...
IO._fetchTimezones = function (events, TZManager) {
	...
	events.forEach(function (event) {
		...
		if (event.dtstart) {
			...
			dtstartYear = dtstart.getUTCFullYear();
			if (!dtendYear) {
				dtendYear = dtstartYear;
			}
		}
...
IO.normalizeToLocalTimezone = function (events, TZManager) {
	"use strict";
	return IO.normalizeToTimezone(events, TZManager, "local");
};

IO.normalizeToEventTimezone = function (events, TZManager) {
	"use strict";
	return IO.normalizeToTimezone(events, TZManager, "event");
};

IO.normalizeToTimezone = function (events, TZManager, direction) {
	"use strict";
	...
	future.then(function (future) {
		...
		events.forEach(function (event) {
			...
			var newDtend, oldDtstart, dt, source = event.tzId, target = TZManager.timezone;
			if (direction === "event") {
				target = event.tzId;
			...
			if (event.dtstart) {
				Utils.debug("----CONVERTING TZ from " + source + " to " + target);
				oldDtstart = event.dtstart;
			...
			IO._normalizeToLocalTimezoneArray(event, IO._EXDATES_PROP_NAME, TZManager, direction);
			IO._normalizeToLocalTimezoneArray(event, IO._RDATES_PROP_NAME, TZManager, direction);
		});
...
IO._normalizeToLocalTimezoneArray = function (event, field, TZManager, direction) {
	"use strict";
	if (event[field]) {
		var i,
			oldDate,
		...
		Utils.debug("----CONVERTING " + field + " TZ from " + source + " to " + target + "; " + stringify(event[field]));
		...
		for (i = 0; i < event[field].length; i += 1) {
			...
			if (lastChar !== "Z" || lastChar !== "z") {
				...
				newDate = new Date(TZManager.convertTime(oldDate.getTime(), source, target));
				value = Utils.dateToIso8601(newDate, {
					noPunct: true,
					noMS: true,
					noTzOffset: true
				});
...
IO._propagateEventPropertiesToExceptions = function (calendar) {
	...
	calendar.events.forEach(function (event) {
		...
		eventGroup = eventHash[event.uid] || {
			exceptions: []
		};
	...
	function propagateToOneEvent(parent, event) {
		var key;
		for (key in parent) {
	...
	for (uid in eventHash) {
		...
		if (eventGroup.exceptions.length && eventGroup.parentEvent) {
			eventGroup.exceptions.forEach(propagateToOneEvent.bind(this, eventGroup.parentEvent));
		}
	}
	/*jslint forin: false */
};
